...
**arXiv ID**: 2512.03560v1
**Authors**: Gianni Molinari, Fabio Ciravegna
**Published**: 2025-12-03
**URL**: http://arxiv.org/abs/2512.03560v1
**Source**: ar5iv full-text rendering

[2512.03560] Reason-Plan-ReAct: A Reasoner-Planner Supervising a ReAct Executor for Complex Enterprise Tasks
...
*[Figure: Figure 1: A PEA receives the sub-question from the RPA and tries to solve it by interacting with tools using the ReAct approach, then returns the resu]*
...
*[Figure: Table 1: `Accuracy` on easy Benchmarks.]*


*[Figure: Table 2: `Accuracy` on hard Benchmarks.]*
...
Considering `M` as the set of models, we evaluate an architecture \in A` with the following metrics:
[table]
...
*[Figure: Table 3: `Mean` ( `\uparrow` ), `StandardDeviation` ( `\downarrow` ) and `CombinedPerformanceScore` ( `\uparrow` ) of each Agent Approach]*


*[Figure: (b) hard Benchmarks]*
...
In Table 2 and 2 we report the accuracy for the easy and hard benchmarks respectively.
...
As we can see in Table 2 and 2 (where P marks specific domain examples included in the prompt), RP-ReAct demonstrates a significantly more consistent performance and superior generalization ability, even when specific examples are omitted from the prompt.
...
In general, as we can see at the bottom of Tables 2 and 2, smaller models (with less than 10B parameters) are still unable to solve tasks consistently regardless of the approach.
...
As explained in the previous Section, we conduct the analysis separating on the easy (Table 3(a)) and the hard benchmark (Table 3(b)).
