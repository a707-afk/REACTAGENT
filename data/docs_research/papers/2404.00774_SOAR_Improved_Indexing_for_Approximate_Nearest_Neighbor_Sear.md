...
[2404.00774] SOAR: Improved Indexing for Approximate Nearest Neighbor Search
...
These large datasets, in conjunction with the curse of dimensionality, a phenomenon which often makes it impossible to find the exact nearest neighbors without resorting to a linear scan, has led to a focus on approximate nearest neighbor (ANN) search, which can trade off a small search accuracy loss for a significant increase in search throughput.
...
We show that the naive training of multiple VQ indices for a single dataset leads to correlation in failure probabilities among the indices, limiting ANN search efficiency uplift, and present spilling with orthogonality-amplified residuals (SOAR) to ameliorate this correlation.
...
•
 ...
 `\pi(v):\mathbb{R}^{d}\mapsto\{1,\ldots,c\}` , the partition assignments that map each vector in `\mathcal{X}` to one of the partition centers in `\mathcal{C}` . Oftentimes, `\pi(v)` is defined as `\operatorname*{arg\,min}_{i\in\{1,\ldots,c\}}\norm{v-\mathcal{C}_{i}}^{2}` , although other assignment functions may also be used.


We can then construct an inverted index over `\pi` ; for each partition `i` , we store the set of datapoint indices belonging to that partition: `\{j|\pi(\mathcal{X}_{j})=i\}` . Then, instead of computing `\textsc{Mips${}_{k}$}(q,\mathcal{X})` directly, we may first compute `\textsc{Mips${}_{k^{\prime}}$}(q,\mathcal{C})` , which is much faster because `|\mathcal{C}|\ll|\mathcal{X}|` . We can then use our inverted index data structure to further evaluate the datapoints within the top `k^{\prime}` partitions.
...
###
3.1 Search difficulty and quantized score error


*[Figure: Figure 1: Greater search difficulty, as quantified by a higher `\textsc{Rank}(q,\mathcal{C}_{\pi(x)},\mathcal{C})` , is associated with highly positi]*


For a datapoint `x` , define ` as the partitioning residual, equal to `x-\mathcal{C}_{\pi(x)}` . The partition center `\mathcal{C}_{\pi(x)}` is the quantized form of `x` , so the difference between the exact and the quantized inner product scores is
[table]


which we denote the quantized score error. Consider the case when `x` is a nearest neighbor for the query: `x\in\textsc{Mips${}_{k}$}(q,\mathcal{X})` . In this scenario, we would like `\textsc{Rank}(q,\mathcal{C}_{\pi(x)},\mathcal{C})` to be low, so that the algorithm may search just the top few partitions and find the nearest neighbor `x` . A high Rank would lead to increased search difficulty, because the algorithm would have to search more partitions and therefore do more work to find `x` .
...
###
3.2 Quantized score error decomposition
...
where `\theta` is the angle formed between the query and the partitioning residual. Without loss of generality, we may assume that `\norm{q}=1` without any effect on the ranking of MIPS nearest neighbors. The two contributors to a highly positive `\left\langle q,r\right\rangle` are therefore a large `\norm{r}` and `\cos\theta` being near unity; reducing `\left\langle q,r\right\rangle` requires targeting either, or both, of these contributors.
...
The VQ training loss already aims to minimize `\mathbb{E}[\norm{x-\mathcal{C}_{\pi(x)}}^{2}]=\mathbb{E}[\norm{r}^{2}]` , so further reductions in `\norm{r}` are difficult. In contrast, `\cos\theta` is not directly optimized for, making it more amenable to reduction.


•


 `\cos\theta` is typically much more strongly correlated with `\left\langle q,r\right\rangle` than `\norm{r}` is, so its reduction has greater impact on search difficulty. Qualitatively, this stronger correlation can be explained by the far greater range of values `\cos\theta` may take on, compared to `\norm{r}` . The latter’s values are concentrated between 0 and `\norm{x}` , with not a very large ratio between the largest and smallest value. In comparison, `\cos\theta` can take on any value in the range `[-1,1]` ; only this large multiplicative dynamic range can explain the variance in `\left\langle q,r\right\rangle` . Empirically, this can be seen demonstrated on Glove-1M in Figure 2 below.


*[Figure: Figure 2: The cosine of the query-residual angle, `\cos\theta` (left), is far more correlated with `\left\langle q,r\right\rangle` than the residu]*


###
3.3 Spilled VQ assignment


We may attempt to mitigate the increase in search difficulty presented by high `\cos\theta` by assigning each datapoint `x` to a second VQ partition `\pi^{\prime}(x)` , resulting in a second partitioning residual ^{\prime}` that forms an angle `\theta^{\prime}` with the query. We denote this second assignment a spilled assignment.


*[Figure: Figure 3: Naive spilled VQ assignment may be ineffective; selecting the two closest centroids `\mathcal{C}_{1}` and `\mathcal{C}_{2}` provides no ]*


This gives our ANN algorithm a “second chance"; assuming `\theta` and `\theta^{\prime}` are independently distributed, the situations where `\cos\theta` is high, thereby leading to high `\left\langle q,r\right\rangle` and greater search difficulty, are unlikely to also have a high `\cos\theta^{\prime}` . A low `\cos\theta^{\prime}` should lead to a low `\left\langle q,r^{\prime}\right\rangle` and should allow the ANN algorithm to find `x` fairly efficiently. This idea can be extended to further (>2) assignments as well.
...
This proposed approach fails when there is correlation between `\cos\theta` and `\cos\theta^{\prime}` , shown to the extreme in Figure 3. In this figure, if `\pi(x)=1` , we may pick `\pi^{\prime}(x)=2` because `\mathcal{C}_{2}` is the second-closest centroid to `x` . However, `\mathcal{C}_{2}` is collinear with `\mathcal{C}_{1}` and `x` , resulting in `\theta=\theta^{\prime}` . This leads to `\mathcal{C}_{2}` acting as a strictly worse version of `\mathcal{C}_{1}` for quantizing `x` : `\left\langle q,r^{\prime}\right\rangle=(1+\epsilon)\left\langle q,r\right\rangle` . The choice `\pi^{\prime}(x)=3` would’ve been more effective, despite `\norm{x-\mathcal{C}_{3}}>\norm{x-\mathcal{C}_{2}}` , due to `\mathcal{C}_{3}` ’s position giving a `\theta^{\prime}\neq\theta` .


*[Figure: (a) `\theta` and `\theta^{\prime}` come from a single VQ index, and are the closest and second-closest centroids, respectively.]*


*[Figure: (b) `\theta_{1}` and `\theta_{2}` come from two VQ indices, trained with different random seeds.]*
...
Experiments show that more than a theoretical concern, this is also a real-world issue. On the Glove-1M dataset, if we choose `\pi(x)` and `\pi^{\prime}(x)` to be the closest and second-closest centroids by Euclidean distance, respectively, to `x` , we find a noticeable correlation between `\cos\theta` and `\cos\theta^{\prime}` , shown in Figure 4(a). An even stronger correlation occurs between two VQ indices trained separately with different random seeds (Figure 4(b)). This correlation diminishes the benefit of spilled assignment, and reducing this correlation is critical to improving ANN search performance.


###
3.4 Spilling with orthogonality-amplified residuals


We now derive spilling with orthogonality-amplified residuals (SOAR), a novel VQ assignment loss that directly confronts the problem of correlated query-residual angles. To derive this loss, assume that the VQ centroids `\mathcal{C}` and assignments `\pi` are fixed. The goal of further spilled assignments `\pi^{\prime}` is to achieve low quantized score error `\left\langle q,r^{\prime}\right\rangle` specifically when the original spilled assignment has high `\cos\theta` , leading to high `\left\langle q,r\right\rangle` . In cases where `\cos\theta` is low, the original assignments already achieve low quantized score error, so in such cases, it is less important to also have low `\left\langle q,r^{\prime}\right\rangle` .


We now modify our assignment loss function to reflect this emphasis; rather than selecting `c^{\prime}\in\mathcal{C}` to minimize `\mathbb{E}_{q\in\mathcal{Q}}[(\left\langle q,x\right\rangle-\left\langle q,c^{\prime}\right\rangle)^{2}]=\mathbb{E}_{q\in\mathcal{Q}}[\left\langle q,r^{\prime}\right\rangle^{2}]` , we now also add a weighting term for `\cos\theta` :
[table]
...
By leveraging Theorem 3.1, we may efficiently perform spilled VQ assignment using our new loss by computing the squared `\ell_{2}` distance to each centroid, adding a penalty term for parallelism between the original residual ` and the candidate residual ^{\prime}` , and taking the `\operatorname*{arg\,min}` among all centroids. We present the following statements to help develop intuition about the SOAR loss:
...
For the uniform weight function `w(t)=1=|t|^{0}` , the SOAR loss is equivalent to standard Euclidean assignment: `\mathcal{L}(r^{\prime},r,\mathcal{Q})\propto\norm{r^{\prime}}^{2}` .
...
For a fixed `\norm{r^{\prime}}` , `\mathcal{L}` is minimized when ` and ^{\prime}` are orthogonal, in which case `\lambda\norm{\operatorname{proj}_{r}r^{\prime}}^{2}=0` and `\mathcal{L}(r^{\prime},r,\mathcal{Q})\propto\norm{r^{\prime}}^{2}` .
...
Lemma 3.2.
 ...
 `\norm{\operatorname{proj}_{r}r^{\prime}}=\norm{r^{\prime}}\cdot\rho_{\left\langle q,r\right\rangle,\left\langle q,r^{\prime}\right\rangle}` ,
where `\rho` is the Pearson correlation coefficient, and the correlation is computed over `q` uniformly distributed over the hypersphere.
...
It’s well known that for uniformly distributed, unit-norm `q\in\mathbb{R}^{d}` that `\mathbb{E}[qq^{T}]=I_{d}/d` , where `I_{d}` is the `d` -dimensional identity matrix, and that `\sigma_{\left\langle q,v\right\rangle}=\norm{v}/\sqrt{d}` for any fixed `v` . We leverage this to get
[table]
...
When the ANN index’s highest-bitrate datapoint representation is int8 or float32, the relative index size growth is therefore `\dfrac{4+\frac{d}{2s}}{d+4+\frac{d}{2s}}\approx\dfrac{\frac{d}{2s}}{d+\frac{d}{2s}}=\dfrac{1}{2s+1}` or `\dfrac{4+\frac{d}{2s}}{4d+4+\frac{d}{2s}}\approx\dfrac{\frac{d}{2s}}{4d+\frac{d}{2s}}=\dfrac{1}{8s+1}` , respectively. Table 1 empirically corroborates these estimates and shows the memory cost is indeed quite low (5%-20% relative increase over a non-spilled VQ index).


*[Figure: Figure 5: Memory layout changes for a SOAR-enabled ANN index. Memory footprint is proportional to area of colored cell. VQ centroid data (not shown) r]*
...
Spill trees (or sp-trees) were originally introduced in [12], and also assign datapoints to multiple vertices at the same level of the tree, just as SOAR may assign a datapoint to multiple VQ partitions. However, there are significant differences; spill trees were searched using defeatist search, a greedy, no-backtracking strategy that took the most promising root-to-leaf path. SOAR indices utilize backtracking in their search procedure; multiple partitions, not just the top one, are evaluated further.
...
###
5.1 KMR curve comparison


The KMR curve is a deterministic and easily computable metric for VQ-style ANN indices that’s highly predictive of real-world performance. Below, we plot `\textsc{Kmr${}_{k}$}(t)` as defined in Equation 1, but rather than plotting with respect to ` , we plot with respect to the sum of the sizes of the ` top-ranked partitions. This partition size weighting is necessary because spilled VQ data structures have more points per partition due to their multiple assignments, making each individual partition more expensive to search. The results are shown in Figure 6; further details are given in Appendix A.2.


*[Figure: (a) Glove-1M]*


*[Figure: (b) Microsoft SPACEV]*


*[Figure: (c) Microsoft Turing-ANNS]*


Figure 6: SOAR allows a VQ index to achieve a given recall target while reading fewer total datapoints and therefore utilizing less memory bandwidth, increasing ANN search performance. SOAR’s improvements are especially large on Microsoft SPACEV and Microsoft Turing-ANNS, the billion-scale datasets in this experiment; see Section 5.3 for further analysis of dataset size.
...
*[Figure: Figure 7: Scatter plot, analogous to Figure 4(a), but with spilled assignments performed using SOAR loss; angular correlation is much lower.]*


*[Figure: Figure 8: The nearest neighbors most difficult to find under partitioning `\pi` , as quantified by `\textsc{Rank}(q,\mathcal{C}_{\pi(x)},\mathcal{C}]*
...
*[Figure: Figure 9: Raising SOAR `\lambda` increases VQ distortion `\mathbb{E}[\norm{r^{\prime}}^{2}]` , but lowers score correlation `\rho_{\left\langle q,]*


Next, in Figure 9, we investigate the effect of SOAR’s `\lambda` parameter on the resulting VQ index. We can see that as `\lambda` is increased, the resulting partitioning `\pi^{\prime}` from SOAR will have quantized score error `\left\langle q,r^{\prime}\right\rangle` less and less correlated with the quantized score error `\left\langle q,r\right\rangle` from `\pi` . This is beneficial for ANN search efficiency, but on the other hand, `\mathbb{E}\left[\norm{x-\mathcal{C}_{\pi^{\prime}(x)}}^{2}\right]=\mathbb{E}\left[\norm{r^{\prime}}^{2}\right]` increases with respect to `\lambda` . The diminishing relative importance of `\norm{r^{\prime}}^{2}` as `\lambda` increases is a natural consequence of Theorem 3.1, but nonetheless results in a general increase in magnitude of `\left\langle q,r^{\prime}\right\rangle` and is detrimental to ANN search accuracy; setting the optimal `\lambda` requires balancing these effects.


###
5.3 Effects of dataset size and recall target


*[Figure: Figure 10: SOAR’s ANN search efficiency benefit grows as SOAR is applied to larger datasets, and as the ANN recall target rises.]*


In Figure 10, we take various size samples from the ann-benchmarks.com DEEP dataset and compare the ratio of how many datapoints a SOAR VQ index must access compared to a standard VQ index to achieve the same recall. This ratio is the dependent variable in Figure 10; a higher ratio indicates a greater improvement from SOAR. We can see that higher recall targets, and larger samples, lead to greater ratios.
...
###
5.4 End-to-end recall-speed benchmarks


*[Figure: Figure 11: SOAR + ScaNN on Glove-1M.]*
...
*[Figure: (a) Throughput-to-capex ratio (higher is better)]*


*[Figure: (b) Throughput-to-cloud bill ratio (higher is better)]*


Figure 12: SOAR leads big-ann-benchmarks performance under both methods of comparison.


Finally, we present memory consumption figures for the ANN indices in these benchmarks in Table 1. The minor increase in memory usage is discussed and analyzed in Section 3.5, and we find that analysis corroborates well with the empirical measurements below.
For all three datasets, the index size increase is very minor, and the throughput gains from SOAR could allow the same query traffic to be served by fewer server replicas, in fact potentially reducing net memory consumption.


*[Figure: Table 1: ANN index memory consumption before/after SOAR.]*
...
Intel® optane™ business update: What does this mean for warranty and
support.
...
Koenigstein, Nir Nice, and Ulrich Paquet.


Speeding up the xbox recommender system using a euclidean
transformation for inner-product spaces.


In Proceedings of the 8th ACM Conference on Recommender
Systems, RecSys ’14, page 257–264, New York, NY, USA, 2014. Association
...
International Conference on Machine Learning, volume 162 of Proceedings
of Machine Learning Research, pages 2206–2240. PMLR, 17–23 Jul 2022.


[6]

Thomas Dean, Mark A. Ruzon, Mark Segal, Jonathon Shlens, Sudheendra
...
Recognition, pages 1814–1821, 2013.
...
on Knowledge Discovery & Data Mining, KDD ’19, page 2509–2517, New York,
...
37th International Conference on Machine Learning, volume 119 of Proceedings of Machine Learning Research, pages 3887–3896. PMLR, 13–18 Jul
2020.
...
Natural Language Processing (EMNLP), pages 6769–6781, Online, November
2020. Association for Computational Linguistics.


[11]

Herwig Lejsek, Björn Þór Jónsson, and Laurent Amsaleg.


Nv-tree: Nearest neighbors at the billion scale.


In Proceedings of the 1st ACM International Conference on
Multimedia Retrieval, ICMR ’11, New York, NY, USA, 2011. Association for
Computing Machinery.


[12]

Ting Liu, Andrew Moore, Ke Yang, and Alexander Gray.
...
Information Processing Systems, volume 17. MIT Press, 2004.
...
IEEE Trans. Pattern Anal. Mach. Intell., 42(4):824–836, apr
2020.
...
In Z. Ghahramani, M. Welling, C. Cortes, N. Lawrence, and K.Q.
...
Harsha Vardhan Simhadri, George Williams, Martin Aumüller, Matthijs Douze,
Artem Babenko, Dmitry Baranchuk, Qi Chen, Lucas Hosseini, Ravishankar
Krishnaswamny, Gopal Srinivasa, Suhas Jayaram Subramanya, and Jingdong Wang.


Results of the neurips’21 challenge on billion-scale approximate
nearest neighbor search.


In Douwe Kiela, Marco Ciccone, and Barbara Caputo, editors, Proceedings of the NeurIPS 2021 Competitions and Demonstrations Track,
volume 176 of Proceedings of Machine Learning Research, pages 177–189.
PMLR, 06–14 Dec 2022.
...
To evaluate this integral, first decompose `q` and ^{\prime}` into components parallel and orthogonal to ` ; let us call these components `q_{\parallel}` , `q_{\perp}` , ^{\prime}_{\parallel}` , and ^{\prime}_{\perp}` , respectively. This allows us to simplify the inner expectation:


[table]


Meanwhile, `dP\left[\dfrac{\left\langle q,r\right\rangle}{\norm{r}\cdot\norm{q}}<\cos\theta\right]` is proportional to the surface area of a `(d-1)` -dimensional hypersphere of radius `\sin\theta` , which we may express as `A\sin^{d-2}\theta` for some constant `A` . Our integral then becomes


[table]


Now define `I_{d}=A\int_{0}^{\pi}w(\cos\theta)\sin^{d}\theta` ; note that `\mathcal{L}(r^{\prime},r,\mathcal{Q})=(I_{d-2}-I_{d})\norm{r^{\prime}_{\parallel}}^{2}+I_{d}\norm{r^{\prime}_{\perp}}^{2}/(d-1)` . For the weight function `w(t)=\absolutevalue{t}^{\lambda}` , we find that `I_{d}` has a recursive definition if we utilize integration by parts with `v=\cos\theta` and `u=-\cos^{\lambda}\theta\sin^{d-1}\theta` :
...
Note that ^{\prime}_{\parallel}=\operatorname{proj}_{r}r^{\prime}` , so this is our desired result. This is very similar to the analysis behind Theorem 3.3 of [8].


###
A.2 KMR curve comparison: further details
...
*[Figure: Table 2: KMR curve results in tabulated form; see Figure 6 for graphs of this data.]*


###
A.3 ann-benchmarks.com Glove-1M benchmark details
...
###
A.4 End-to-end recall-speed benchmarks: further details
...
1 x Intel® Xeon® Silver 4314 Processor 16-Core 2.40 GHz 24MB Cache (135W)


•


6 x 32GB DDR4 3200MHz ECC RDIMM Server Memory (2Rx8 - 16Gb)
