...
When using `must`, the clause becomes rue` only if every condition listed inside `must` is satisfied.
...
When using `should`, the clause becomes rue` if at least one condition listed inside `should` is satisfied.
...
When using `must_not`, the clause becomes rue` if none of the conditions listed inside `must_not` is satisfied.
...
In this example, the condition will be satisfied if the stored value is either lack` or `yellow`.
...
In this example, the condition will be satisfied if the stored value is neither lack` nor `yellow`.
...
It is achieved by using the ested` condition type formed by a payload key to focus on and a filter to apply.
...
A special case of the `match` condition is the ext` match condition.
...
The ext_any` full-text match condition is similar to the ext` condition, but with a key difference: while ext` only matches text fields that contain all the query terms, ext_any` matches fields that contain any of the query terms. In other words, even if a text field contains just one of the query terms, it is considered a match.
...
The ange` condition sets the range of possible values for stored payload values.
If several values are stored, at least one of them should match the condition.
...
It matches with `location`s inside a rectangle with the coordinates of the upper left corner in op_left` and the coordinates of the lower right corner in ottom_right`.
...
It matches with `location`s inside a circle with the `center` at the center and a radius of adius` meters.
...
This condition will match all records where the field eports` either does not exist, or has ull` or `[]` value.
...
This condition will match all records where the field eports` exists and has `NULL` value.
...
Refer to A Complete Guide to Filtering in Vector Search for developer advice on proper usage and advanced practices.
