<!-- source: Milvus milvus_product_faq.md --><!-- fetched: 2026-06-14 -->---
id: product_faq.md
title: Product FAQ
---
...
#### Why do I get fewer than k vectors when searching for opk` vectors?

Among the indexes that Milvus supports, IVF_FLAT and IVF_SQ8 implement the k-means clustering method. A data space is divided into list` clusters and the inserted vectors are distributed to these clusters. Milvus then selects the probe` nearest clusters and compares the distances between the target vector and all vectors in the selected clusters to return the final results.

If list` and opk` are large and nprobe is small, the number of vectors in the nprobe clusters may be less than `k`. Therefore, when you search for the opk` nearest vectors, the number of returned vectors is less than `k`.

To avoid this, try setting probe` larger and list` and `k` smaller.
