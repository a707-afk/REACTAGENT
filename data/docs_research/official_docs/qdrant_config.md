...
<!-- url: https://qdrant.tech/documentation/ops-configuration/configuration/ -->
<!-- fetched: 2026-06-14 -->
...
Main Configuration: `qdrant/config/config.yaml`
...
Environment-Specific Configuration: `config/{RUN_MODE}.yaml`
...
Custom Configuration via `--config-path`
...
# logger:
# # Logging format, supports ext` and `json`
# format: text
# on_disk:
# enabled: true
...
storage:
 ...
 # If ull` - maximum concurrency is used.
 ...
 # Note: 1Kb = 1 vector of size 256
 # To explicitly disable vector indexing, set to `.
...
service:
 ...
 # If ull` - gRPC is disabled. Default: null
